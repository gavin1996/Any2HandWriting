...
class Model(object):
    ...
    def __init__(self,
        ...
        if ckpt and load_model:
            ...
            logging.info("Reading model parameters from %s finished!" % ckpt.model_checkpoint_path)
        else:
    ...
    def launch(self):
        ...
        if self.phase == 'test':
            ...
            if num_total != 0:
                logging.info('Total correct: %.2f' %(count_correct/num_total*100))
        elif self.phase == 'evaluate':
            num_correct = 0
            num_total = 0
            count_correct = 0
            fword = open(os.path.join(self.data_base_dir, 'predict.txt'), 'w')
            for batch in self.s_gen.gen(self.batch_size):
                # Get a batch and make a step.
                start_time = time.time()
                bucket_id = batch['bucket_id']
                ...
                for idx, output, ground in zip(range(len(grounds)), step_outputs, grounds):
                    ...
                    for j in range(1,len(ground)):
                        ...
                        if s1 != 2 and flag_out:
                            ...
                            flag_out = False
                    fword.write(file_list[idx][file_list[idx].find("\\")+1:])
                    fword.write(' ')
                    for c in output_valid:
                        if c>2 and c<13:
                            fword.write(chr(c-3+48))
                        elif c>12 and c<39:
                            fword.write(chr(c-13+97))
                        else:
                            fword.write(chr(c-39+65))
                    fword.write('\n')
                    print(file_list[idx])
                logging.info('Number: %d' %(num_total))
            if num_total != 0:
